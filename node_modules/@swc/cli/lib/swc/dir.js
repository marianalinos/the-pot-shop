...
async function watchCompilation(cliOptions, swcOptions, callbacks) {
    ...
    const watcher = await (0, _sources.watchSources)(filenames, includeDotfiles, only, ignore);
    watcher.on("ready", ()=>{
    ...
    watcher.on("unlink", async (filename)=>{
        try {
            if ((0, _sources.isCompilableExtension)(filename, extensions)) {
                await unlink((0, _util1.getDest)(filename, outDir, stripLeadingPaths, `.${(0, _util1.mapTsExt)(filename)}`));
                const sourcemapPath = (0, _util1.getDest)(filename, outDir, stripLeadingPaths, `.${(0, _util1.mapTsExt)(filename)}.map`);
